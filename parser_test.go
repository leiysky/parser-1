...
func (s *testParserSuite) TestBuiltin(c *C) {
	table := []testCase{
		...
		{`select approx_percentile(c1, 50) from t;`, true, "SELECT APPROX_PERCENTILE(`c1`, 50) FROM `t`"},
		{`select approx_percentile(c1, 1.1) from t;`, true, "SELECT APPROX_PERCENTILE(`c1`, `c2`) FROM `t`"},
